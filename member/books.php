...
<html>
	...
	<body>
		...
		<?php
			...
			if($rows == 0)
				echo "<h2 align='center'>Trenutna nije izdata nijedna knjiga</h2>";
			else
			{ 
				echo "<form class='cd-form' method='POST' action='#'>";
				echo "<legend>Knjige</legend>";
				...
				echo "</table><br />";
				echo "<input type='submit' name='b_return' value='Vrati knjigu' />";
				echo "</form>";
			}
			...
			if(isset($_POST['b_return']))
			{
				...
				for($i=0; $i<$rows; $i++)
					if(isset($_POST['cb_book'.$i]))
					{
						...
						$query->bind_param("ss", $_SESSION['username'], $_POST['cb_book'.$i]);
						if(!$query->execute())
							die(error_without_field("Nije moguće vratiti knjigu!"));
						...
						if($days > 0)
						{
							...
							$query->execute();
							echo '<script>
									document.getElementById("error").innerHTML += "'.$penalty.' je naplaćemo zbog zadržavanja knjige '.$_POST['cb_book'.$i].'.<br />";
				...
				{
					echo '<script>
							document.getElementById("success").innerHTML = "Uspešno vraćena knjiga '.$books.'";
					...
					echo error_without_field("Označite knjigu za vraćanje");
			}
		?>
